...
class MVARAutoGenAdapter(MVARExecutionAdapter):
    """Wrapper for AutoGen-style tool call payloads."""
    ...
    def execute_tool_call(
        ...
        if not tool_name or tool_name not in tool_registry:
            raise ValueError(f"Unknown AutoGen tool: {tool_name}")

        raw_args = function_obj.get("arguments", tool_call.get("arguments", {}))
        if raw_args is None:
